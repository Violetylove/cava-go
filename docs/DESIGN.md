...
- **交叉编译**：无 cgo，任意平台 `CGO_ENABLED=0 GOOS=linux` 直接产出静态 ELF（CI：`release` job 用 softprops 建 Release，`build` job 用 `wangyoucao577/go-release-action` 矩阵 windows/amd64 + linux/amd64 交叉编译并上传，docker action 仅限 ubuntu runner；`test` job 双平台原生跑单测）；
