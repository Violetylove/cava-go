...
- **Linux 构建**：无外部依赖（纯 Go native protocol，无需 libpulse-dev）；`CGO_ENABLED=0 GOOS=linux` 任意平台交叉编译；CI：`test` job 双平台（windows/ubuntu）原生跑 vet/test，`release` job 用 **softprops 建 Release**，`build` job 用 **`wangyoucao577/go-release-action` 矩阵**（windows/amd64 + linux/amd64，docker action 仅限 ubuntu runner）交叉编译并上传 Release assets
